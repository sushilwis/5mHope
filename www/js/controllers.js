angular.module('5mHope.controllers', [])
  ...
  .controller("BrowseCtrl", function ($scope, $timeout, $stateParams, $state, $ionicModal, AudioSvc, $ionicPlatform, $fileFactory, $window, $rootScope, $ionicLoading, $ionicScrollDelegate, $ionicHistory) {
        ...
        function plsuccess(data) {
          ...
          AudioSvc.playAudio(src, function (a, b) {
            ...
            if (parseInt(play_time) > 0) {
              ...
              current_play_time = (parseInt(b) * parseInt(play_time)) + parseInt(inner_a_time);
            }
            ...
            $scope.songduetime = "-" + convertSeconds(($scope.songTime * 60) - elapsedtime);
    ...
    function setMarkers(map) {
      ...
      for (var i = 0; i < $scope.groups.length; i++) {
        ...
        var marker = new google.maps.Marker({
          position: {
            lat: parseInt(beach.lat),
            lng: parseInt(beach.lng)
          },
          map: map,
          //icon: image,
          shape: shape,
  ...
  .controller('AddGroupCtrl', function ($scope, $stateParams, $state, $http, APP_CONSTANTS, $ionicPopup) {
    ...
    $scope.addgroup = function () {
      ...
      if ($scope.val.groupname == null || $scope.val.zipcode == null || $scope.val.city == null || $scope.val.state == null || $scope.val.country == null) {
  ...
  .controller('PrayerSongsCtrl', function ($scope, $state, $http, APP_CONSTANTS, $stateParams, $rootScope) {
    $scope.$on('$ionicView.beforeEnter', function () {
  ...
  .controller('LoadScripturesCtrl', function ($scope, $state, $http, APP_CONSTANTS, $ionicLoading, $stateParams) {
    ...
    $scope.booksList = [{
      ...
      {
        title: '7. 1 Corinthians',
        value: '1Corinthians'
      },
      {
        title: '8. 2 Corinthians',
        value: '2Corinthians'
      },
      {
        title: '9. Galatians',
      ...
      {
        title: '13. 1 Thessalonians',
        value: '1Thessalonians'
      },
      {
        title: '14. 2 Thessalonians',
  ...
  .controller('BecomeLeaderCtrl', function ($scope, $state, $http, APP_CONSTANTS, $ionicPopup) {
    ...
    $scope.becomeleader = function (selected_id) {
      ...
      $http({
          method: 'POST',
          url: APP_CONSTANTS.API_URL + APP_CONSTANTS.MAKEMELEADER_URL + localStorage.getItem("id") + '&userId=' + localStorage.getItem("userId") + '&leaderTypeId=' + selected_id,
          data: data
        }).success(function (data, status) {
          if (status == 200) {
  ...
  .controller('UpdateProfileCtrl', function ($scope, $stateParams, $state, $http, APP_CONSTANTS, $ionicPopup) {
    ...
    $scope.updateDetails = function () {
      ...
      $http({
          method: 'POST',
          url: APP_CONSTANTS.API_URL + APP_CONSTANTS.UPDATEDETAILS_URL + localStorage.getItem("id") + '&firstName=' + $scope.data.firstname + '&lastName=' + $scope.data.lastname + '&address=' + $scope.data.address + '&state=' + $scope.data.state + '&zip=' + $scope.data.zip + '&phone=' + $scope.data.phone + '&latitude=' + $scope.latitude + '&longitude=' + $scope.longitude,
          headers: {
            'Content-Type': 'application/json'
          },
          data: data
  ...
  .controller('NotificationsCtrl', function ($scope, $stateParams, $http, APP_CONSTANTS, $ionicPopup, $state) {
    ...
    $scope.callback = function (id, sentBy, notificationStatus, groupID, userId) {

      var myuserId = localStorage.getItem("userId");

      $http({
        method: 'GET',
        url: APP_CONSTANTS.API_URL + APP_CONSTANTS.GROUPNAME_URL + groupID + '?access_token=' + localStorage.getItem("id"),
        ...
        if (status == 200) {
          ...
          if (sentBy != "Leader") {
            ...
            $http({
              method: 'GET',
              url: APP_CONSTANTS.API_URL + APP_CONSTANTS.USERNAME_URL + sentBy + '?access_token=' + localStorage.getItem("id"),
            }).success(function (data, status) {
